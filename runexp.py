...
# Problem parameters
R = [(10,10,10,10,10,10,10)]*3
N = [10000]*3
Corners = [10] * 3
Reps = [100] * 3
Indices = [True,False,True] * 1
Pattern = [True,False,True] * 1
Sparse = [False,True,True]*1
Nag = [False,False,False]*1
# Indices = [True]
# Pattern = [True]
# Sparse = [True]
# Nag = [True]
...
for i in range(len(R)):
    ...
    with open(f'experiments/printout/r_{r}_n_{n}_corners_{corners}_reps_{reps}_indices_{indices}_pattern_{pattern}_sparse_{sparse}_nag_{nag}.txt', 'w') as sys.stdout:
        ...
        for rep in range(reps):
            ...
            print("Running BCG...")
            last_time = time.time()
            psi_n, iter_count, sigd_count, ip_count, as_size, as_drops = nonten(X, Y, r, rng, tol=1e-4, indices=indices, pattern=pattern, sparse=sparse, nag=nag)
            curr_time = time.time()
        ...
        np.save(f'experiments/record/r_{r}_n_{n}_corners_{corners}_reps_{reps}_indices_{indices}_pattern_{pattern}_sparse_{sparse}_nag_{nag}', nonten_results)

        print(f"BCG with {indices} indices, {pattern} pattern, {sparse} sparse, {nag}, nag speed ups:")
